...
export default class RescueQuest extends Quest.Base {
    ...
    public override async start(): Promise<Message<true>> {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor("#4CAF50")
            .setImage("https://cdn.discordapp.com/attachments/1379101132743250082/1383443484333178931/iu.png?ex=684ecfa5&is=684d7e25&hm=b4d74322559c7db6d7a8c8ea90e8e567719ab7d203e5b9fbe1f6ab633d2ed5e5&")
    ...
    public override async end(): Promise<Quest.EndReturn> {
        ...
        if (playersWon)
            users.forEach(async (user) => await user.addGold(Math.floor(Math.random() * (this.maxGoldReward - this.minGoldReward)) + this.minGoldReward).addXP(100).save())
        else 
        ...
        const embed = new EmbedBuilder()
            .setTitle("Result")
            .setDescription(playersWon ? `The players won over the beast!` : `The beast won over the players and ran away with the villager` + `\nBeast strengh: ${beastStrengh}, player strengh: ${playerStrength}`)
            .setColor("#4CAF50")
            .setURL("https://www.youtube.com/@LucasDevelop")
            .toJSON();
