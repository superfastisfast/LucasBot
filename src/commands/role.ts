...
import { SlashCommandBuilder, User, InteractionContextType, type Client, type CommandInteraction } from "discord.js";
...
export default class XpCommand extends Command {
    ...
    override async executeCommand(client: Client, interaction: CommandInteraction<any>): Promise<void> {
        ...
        if (!role || !member) {
            interaction.reply({ content: 'Invalid user or role.', ephemeral: true });
        }
        ...
        switch (sub) {
            case "give": {
                ...
                try {
                    ...
                    interaction.reply({ content: `${interaction.user} gave role ${role.toString()} to ${target}`, flags: 'Ephemeral' });
                } catch (err) {
                    console.error(err);
                    interaction.reply({ content: `${interaction.user} failed to add role ${role.toString()} from user ${target}`, flags: 'Ephemeral' });
                }
            ...
            case "remove": {                
                ...
                try {
                    ...
                    interaction.reply({ content: `${interaction.user} removed role ${role.toString()} from ${target}`, flags: 'Ephemeral' });
                } catch (err) {
                    console.error(err);
                    interaction.reply({ content: `${interaction.user} failed to remove role ${role.toString()} from user ${target}`, flags: 'Ephemeral' });
                }
