...
export default class CCompileCommand extends Command.Base {
    public override main: Command.Command = new Command.Command(
        "ccompile",
        "/ccompile, Then send your code as seperate message",
        [],
    ...
    public async onExecute(interaction: CommandInteraction): Promise<InteractionResponse<boolean>> {
        const resp = await interaction.reply({
            content: "The contents of __**your next**__ message will be compiled to C using `gcc` and executed from a `x86_64 GNU + Linux` machine.",
            flags: "Ephemeral",
        });
        ...
        collector.on("collect", async (msg: Message) => {
            ...
            let anim: NodeJS.Timeout = setInterval(async () => {
                ...
                const content = phase === "compile" ? `[1/2] Compile ${frame}` : `[1/2] Compile / OK\n[2/2] Run ${frame}`;
                try {
            ...
            const compileExit: number = await Promise.race([
                ...
                new Promise<number>((resolve) => {
                    setTimeout(() => {
                        ...
                        try {
                            Bun.spawnSync(["docker", "rm", "-f", containerBuild]);
                        } catch {
                            /* ignore */
                        }
                        resolve(-1);
            ...
            const runExit: number = await Promise.race([
                ...
                new Promise<number>((resolve) => {
                    setTimeout(() => {
                        ...
                        try {
                            Bun.spawnSync(["docker", "rm", "-f", containerRun]);
                        } catch {
                            /* ignore */
                        }
                        resolve(-1);
