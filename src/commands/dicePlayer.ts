...
export default class PingCommand extends Command {
    override get info(): any {
        ...
        return new SlashCommandBuilder()
            ...
            .addUserOption((option) =>
                option.setName("target").setDescription("Whom to challenge").setRequired(true),
            )
    ...
    override async executeCommand(client: Client, interaction: CommandInteraction): Promise<void> {
        ...
        if (amount > 100) {
            interaction.reply({ content: `${amount} can't be over 100g`, flags: 'Ephemeral' });
        }
        ...
        try {
            ...
            console.error(err);
            interaction.reply({ content: `${interaction.user} failed to challange ${target}'s xp to ${amount}`, flags: 'Ephemeral' });
        }
    }
}
