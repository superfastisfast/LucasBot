...
export default class ShopCommand extends Command.Base {
    override get info(): any {
        return new SlashCommandBuilder().setName("shop").setDescription("view the store for epic itmes!").toJSON();
    }
        ...
        for (const item of availableItems) {
            if (interaction.customId === interaction.user.id + item.name) {
                ...
                if (user.database.inventory.gold >= item.cost) {
                    ...
                    await user.addGold(-item.cost).equipItem(item).save();
                }

                const shopDisplay = this.generateShopDisplay(user, availableItems);
    ...
    private generateShopDisplay(user: AppUser, items: ItemDocument[]) {
        ...
        for (const item of items) {
            const cannotAffordButtonEnabled: boolean = user.database.inventory.gold < item.cost ? true : false;
